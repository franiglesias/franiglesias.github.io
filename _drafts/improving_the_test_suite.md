...
El problema obvio que surge es que cuanto mayor es la cantidad de tests más tiempo tarda en ejecutarse la suite completa, lo que ralentiza el proceso de desarrollo retrasando el feedback, la generación de pull requests y el despliegue.

¿En qué aspectos podemos intervenir entonces?

* **Mejorar la velocidad de ejecución de la suite**. Tenemos tests que tienen tiempos de ejecución enormes que no están justificados por el tipo de proceso bajo test. En este caso hemos comprobado usar **prophecy** como framework para generar *test doubles* es el principal cuello de botella. Utilizar los *doubles* "nativos" de **phpunit** mejora el rendimiento en niveles espectaculares. Esta es la medida principal, ya que nos permite mejorar tiempos en saltos enormemente significativos.
* **Mejorar el código de los tests**. En algunos casos se podría decir que nuestro testing arrastra deuda técnica. Dicho de otro modo, no todos nuestros tests están lo bastante bien escritos como para ejecutarse con agilidad, por lo que cada test que toquemos debería ser refactorizado para ser más útil y más eficiente. Las ganancias en performance en este caso son más pequeñas, pero también resultan útiles ya que, en general, nos permiten consumir menos recursos.
...
Los test deberían ser rápidos, particularmente los unitarios. Tener tests muy lentos en la capa de unitarios es un problema y nosotros teníamos algunos tests con duraciones realmente exageradas. Así que empezamos a examinarlos.
...
Los datos así ya resultan bastante impresionantes, aunque se puede ver que hay mucho margen de mejora en el Test Case 3. Sin embargo, hay que decir que en los Test Case 2 y 3 se reescribieron y añadieron nuevos tests, mejorando la cobertura de los casos y la expresividad del propio test. Es decir, incluso con más tests podemos ir más rápido.
...
En este TestCase un objeto utilizado por el *subject under test* se carga mediante una fixture usando Alice/Lakion. El TestCase tiene 103 tests, generados mediante *data providers* porque se trata de un elemento bastante importante para negocio.

El problema es que la fixture se carga cada vez que se ejecuta un test lo que multiplica el tiempo de ejecución. Para esto hemos encontrado dos posibles soluciones:

* Instancia en el setUp un doble del objeto y hacer stub de los métodos necesarios.
...
Una de las cosas que pudimos hacer fue simplificar la creación de stubs ya que teníamos doubles que simulaban todas las llamadas. De este modo, redujimos los stubs sólo a las llamadas realmente necesarias. 

En este contexto surgió una técnica curiosa para testear legacy.
...
Pues bien, basta con convertir los stubs en mocks, añadiendo expects. Si al ejecutar el test falla, entonces puedes eliminar ese stub ya que nunca se llama, al menos en el contexto de ese test.
...
Un caso típico puede ser un test unitario de un servicio que utilizar algún Mailer para enviar un mensaje de correo electrónico. No podemos observar ese efecto para los objetivos del test, tan sólo podemos recoger su respuesta (el Mailer podría devolver un código que nos diga si el envío se ha realizado) o bien asegurarnos de que ha sido llamado por el SUT con los parámetros esperados.

Cualquier cosa que pueda pasar dentro del método send de ese mailer es completamente prescindible para nosotros.

A nivel de tests unitarios no es necesario crear Fakes que tengan comportamiento más o menos real. En consecuencia no necesitamos doblar objetos que el doble usa internamente, así como tampoco necesitamos doblar sus dependencias. Lo único que tenemos que hacer es simular que tiene cierto comportamiento, incluyendo lanzar excepciones, para verificar que nuestro código bajo test reacciona adecuadamente.
