...
Idempotente. Se dice de una función que es **idempotente** cuando al ejecutarla con los mismos argumentos devuelve siempre los mismos resultados.

Nuestro ejemplo no es idempotente, ya que si lo ejecutamos varias veces con el mismo argumento devuelve resultados diferentes cada vez. El caso es que debería serlo ya que si tenemos un precio base y le aplicamos un descuento, el precio final debería ser siempre el mismo. Eso nos indica que hay algo influyendo en esa función que no tendría que estar ahí.
...
Así que, para evitar estos problemas, es frecuente crear Testables que extienden la clase que queremos poner a prueba ofreciendo un valor 
...
Dado que hemos congelado el tiempo en el día 6 de diciembre de 2017, si fijamos la fecha de publicación de nuestro TestablePost al día de Reyes de 2018, se supone que no estará publicado y, por tanto, el test pasará. 
...
## ¿Dos mentiras hacen una verdad o una mentira más gorda?
...
En este caso tenemos otras opciones y una de las más sencillas sería tener un método `merge` o `concat` en la clase StudentCollection, de modo que pudiésemos simplemente concatenar las colecciones de estudiantes encontradas, lo que nos permitiría incluso tener obtenerlas de distintos repositorios.
