...
Consolido estos cambios en un nuevo _commit_, ya que los tests están pasando. Ahora nos quedaría modificarlos tests verificar que ya no es necesario que sigamos pasando `Dungeon` al método `do`.
...
Como hemos visto, Player podría perfectamente notificar eventos como `PlayerAwaked` y `PlayerDied`. Otro vento importante es `PlayerExited` aunque aún no tengo claro qué objeto debería publicarlo. Luego podremos suscribir otros objetos para que los puedan escuchar.
...
No tengo muy claro qué test hacer para desarrollar esto, pero cero que este podría ser un comienzo:
