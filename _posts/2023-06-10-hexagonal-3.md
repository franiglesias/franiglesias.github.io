...
Básicamente, queremos que nos ayude a colocar los paquetes, que vienen en cajas con tamaños estándar[^2], en contenedores optimizando el espacio. Así, los paquetes pueden ser de tres tamaños: 1 volumen, 2 volúmenes y 3 volúmenes.

[^2]: Por simplificar el problema
...
Así que parece que podemos identificar tres actores o roles[^1], en tanto que posibles usuarias del sistema con distintas intenciones y necesidades. Eso es independientemente de que en la vida real podrían ser una misma persona. 

[^1]: Con el uso del término rol quiero significar que una misma persona puede ejecutar los distintos roles.
...
Dado que el patrón de Arquitectura Hexagonal no prescribe ninguna organización de código que debamos seguir, tenemos bastante libertad. Mi preferencia personal sería tener una carpeta `domain`[^5] bajo la que organizar todos estos conceptos a medida que vayan apareciendo en beneficio de la claridad, pues sospecho que acabarán surgiendo varios conceptos y la raíz de la carpeta `app` acabará quedando muy poblada. Pero como digo, es una preferencia personal.

[^5]: Podrías llamar a esta carpeta `business`, `core`, `domain_model`, incluso `hexagon`, si me apuras. El nombre `domain` simplemente significa _aquello de lo que se ocupa la aplicación_ y se utiliza convencionalmente en todo tipo de modelos de arquitectura de software con ese significado, ya sea _DDD_, _Clean Architecture_, _Onion Architecture_, etc.
...
`PackageQueue`[^6] está contribuyendo a la definición del puerto secundario _for_enqueueing_packages_. ¿Debería llamarse `ForEnqueueingPackages`?

[^6]: Debo mencionar que en este punto, me pregunto cuál sería el mejor nombre para esta clase.
