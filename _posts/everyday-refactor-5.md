...
Para empezar, cuando tenemos una variable o propiedad de tipo string, tenemos infinitos posibles valores potenciales de esa variable o propiedad, y tan sólo queremos usar dos de ellos. Así que, cuando necesitemos usarlo, tendremos que asegurarnos de que sólo consideraremos esos dos strings concretos. En otras palabras, tendremos que validarlos cada vez.
...
Pero esto debería hacerse cada vez que vamos a utilizar esta variable o propiedad, o al menos, siempre que sepamos que su origine no es confiable, como el input de usuario o una petición a la API o cualquier fuente de datos que no esté en un estado conocido.
...
Por otro lado, está el tema de las constantes públicas. Es una cuestión de conveniencia, como veremos, ya que nos puede permitir acceder a los valores estándar en momentos en los que no podemos usar objetos.
...
class Status
{
    ...
    public function cancelled(): Status
    {
...
Como hemos dicho, este tipo de reglas de negocio pueden encapsularse en el propio enumerable.
...
Un enfoque pragmático, cuando las combinaciones de valores/versiones son reducidas, sería incorporar esa capacidad al propio Enumerable, mediante un named constructor específico y un método para obtener una versión int del valor.
...
class Status
{
    ...
    public static function fromInt(int $status): Status
    {
        return new self(self::INT_MAP[$status]);
    }

    public function toInt(): int
    {
        return array_flip(self::INT_MAP)[$this->status];
    }
...
Aunque la solución que acabamos de ver resulta práctica en ciertos casos, lo cierto es que no es precisamente rigurosa al mezclar responsabilidades. Una forma de gestionarlo sería mediante composición.

La versión básica contiene la representación canónica de los valores en nuestro sistema. Diversos decoradores nos permitirían lidiar con las traducciones de uno a otro sistema.
